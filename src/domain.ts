...
export type Elim = EApp | EProj | EElimHEq | EElimHEqUnsafe | EIndBool | EIFixInd | EIndType;
...
export const vindtype = (args: Val[]): Val => {
  ...
  if (v.tag === 'VSort' && v.sort === '*') return vapp(rest[1], false, rec());
  ...
  if (v.tag === 'VNe') {
    if (v.head.tag === 'HPrim' && v.head.name === 'UnitType') return vapp(rest[7], false, rec());
    if (v.head.tag === 'HPrim' && v.head.name === 'Bool') return vapp(rest[8], false, rec());
    if (v.head.tag === 'HPrim' && v.head.name === 'IFix') {
      const args = toArray(v.args, x => (x as EApp).arg).reverse();
      return vapp(vapp(vapp(vapp(rest[9], false, rec()), false, args[0]), false, args[1]), false, args[2]);
    }
    if (v.head.tag === 'HPrim' && v.head.name === 'HEq') {
      const args = toArray(v.args, x => (x as EApp).arg).reverse();
      return vapp(vapp(vapp(vapp(vapp(rest[10], false, rec()), false, args[0]), false, args[1]), false, args[2]), false, args[3]);
    }
  ...
  if (t.tag === 'Prim') {
    ...
    if (t.name === 'genindType')
      return VAbs(true, 'P', VPi(false, '_', VType, _ => VType), P =>
        ...
        VAbs(false, 't', VType, t => vindtype([t, P, pt, pp1, pp2, ps1, ps2, ps3, pu, pb, pf, pe])))))))))))));
    return VPrim(t.name);
  }
...
const quoteElim = (t: Term, e: Elim, k: Ix, full: boolean): Term => {
  ...
  if (e.tag === 'EIndType') {
    const [P, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10] = e.args.map(x => quote(x, k, full));
    return App(App(App(App(App(App(App(App(App(App(App(App(Prim('genindType'), true, P), false, p1), false, p2), false, p3), false, p4), false, p5), false, p6), false, p7), false, p8), false, p9), false, p10), false, t);
  }
  return e;
};
...
export const showElimQ = (e: Elim, k: number = 0, full: boolean = false): string => {
  ...
  if (e.tag === 'EIndType') return `(genindtype ${e.args.map(x => showTermQ(x, k, full)).join(' ')})`;
  return e;
};
export const showElim = (e: Elim, ns: List<Name> = Nil, k: number = 0, full: boolean = false): string => {
  ...
  if (e.tag === 'EIndType') return `(genindtype ${e.args.map(x => showTermS(x, ns, k, full)).join(' ')})`;
  return e;
};
