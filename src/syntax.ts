...
export type Term = Var | Global | App | Abs | Let | Con | Case | Pi | Data | Type | Ann | Hole | Meta;
...
export const globalUsed = (k: Name, t: Term): boolean => {
  ...
  if (t.tag === 'Con') return globalUsed(k, t.type) || t.args.some(([x, _]) => globalUsed(k, x));
...
export const indexUsed = (k: Ix, t: Term): boolean => {
  ...
  if (t.tag === 'Con') return indexUsed(k, t.type) || t.args.some(([x, _]) => indexUsed(k, x));
...
export const isUnsolved = (t: Term): boolean => {
  ...
  if (t.tag === 'Con') return isUnsolved(t.type) || t.args.some(([x, _]) => isUnsolved(x));
...
export const toSurface = (t: Term, ns: List<Name> = Nil): S.Term => {
  ...
  if (t.tag === 'Con') return S.Con(toSurface(t.type, ns), t.index, t.total, t.args.map(([x, p]) => [toSurface(x, ns), p]));
...
export const shift = (d: Ix, c: Ix, t: Term): Term => {
  ...
  if (t.tag === 'Con') return Con(shift(d, c, t.type), t.index, t.total, t.args.map(([x, p]) => [shift(d, c, x), p]));
