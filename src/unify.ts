...
import { Term, Var, showTerm, Pi, Abs, App } from './syntax';
...
export const unify = (k: Ix, a_: Val, b_: Val): void => {
  ...
  if (a.tag === 'VPi' && b.tag === 'VPi' && a.plicity === b.plicity) {
    ...
    const s = VVar(k + 1);
    const w = VVar(k + 2);
    return unify(k + 3, a.body(s, v, w), b.body(s, v, w));
  }
...
const solve = (k: Ix, m: Ix, spine: List<Elim>, val: Val): void => {
  ...
  try {
    ...
    const solution = foldl((body, [pl, y]) => {
      if (typeof y === 'string') return Abs(pl, '_', body);
      return Abs(pl, '_', body);
    }, body, spinex);
...
const checkSolution = (k: Ix, m: Ix, is: List<Ix | Name>, t: Term): Term => {
  ...
  if (t.tag === 'Pi') {
    ...
    const body = checkSolution(k + 3, m, Cons(k + 2, Cons(k + 1, Cons(k, is))), t.body);
    return Pi(t.plicity, t.self, t.rec, t.name, ty, body);
  }
